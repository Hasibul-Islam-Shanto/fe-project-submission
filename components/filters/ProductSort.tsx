...
import { SORT_OPTIONS, type SortOptionId } from "@/lib/filters/constants";

const ProductSort = () => {
  const [selected, setSelected] = useState<SortOptionId>("price-asc");
  const [isOpen, setIsOpen] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);

  const selectedLabel =
    SORT_OPTIONS.find((option) => option.id === selected)?.label ?? "Sort by";
